...
function renderDrop(data, placeholder) {
	var options = "";
	for (i = 0; i < data.length; i++) {
		options += '<option value="' + data[i].value + '">' + data[i].label + '</option>'
	}
	var select = '<div class="dropDownMsg"><select class="browser-default dropDownSelect"> <option value="" disabled selected>'+ placeholder +'</option>' + options + '</select></div>'
		...
		if (response.length < 1) {
			...
			for (i = 0; i < response.length; i++) {
				...
				if (response[i].hasOwnProperty("custom")) {
					...
					if (response[i].custom.payload.slice(0,8) == "dropDown") {
						dropDownData = response[i].custom.data;
						renderDropDown(dropDownData, response[i].custom.payload.slice(9,response[i].custom.payload.length));
						return;
					}
...
function renderDropDown(data, placeholder) {
	var options = "";
	for (i = 0; i < data.length; i++) {
		options += '<option value="' + data[i].value + '">' + data[i].label + '</option>'
	}
